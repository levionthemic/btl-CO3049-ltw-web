...
function DetailPage() {
  ...
  const handleBooking = (e) => {
    ...
    bookRoomAPI(bookingData).then((res) => {
      if (res.status === 200) {
        Swal.fire({
  ...
  return (
    <div className='max-w-6xl mx-auto p-6 space-y-4'>
      ...
      <div className='flex flex-col md:flex-row md:items-center md:justify-between'>
        ...
        <div className='text-right flex flex-col items-center mt-4 md:mt-0'>
          <p className='text-3xl font-bold text-mainColor1-900'>
            ${room.price_per_night}
            <span className='text-lg font-normal'>/night</span>
          </p>
          <p className='text-xl font-semibold text-mainColor1-500'>
      ...
      <div className='flex items-center justify-center !mt-10 !mb-10'>
        <form
          ...
          className='bg-white shadow-xl rounded-2xl p-10 w-[90%] max-w-6xl'
        >
          <div className='grid grid-cols-1 md:grid-cols-4 gap-4 items-center'>
            ...
            <button
              type='submit'
              className='bg-mainColor-400 text-white px-4 rounded w-full hover:bg-mainColor-800 hover:scale-105 hover:drop-shadow-lg hover:duration-300 hover:ease-in-out transition-all py-4'
            >
              Book Rooms
            </button>
          </div>
      ...
      {/* Overview */}
      <div className='!mb-10'>
        <h2 className='text-3xl font-semibold mb-2 text-mainColor-600'>
          Description
        </h2>
        ...
        <div className='mt-4 flex flex-wrap gap-4'>
          ...
          {amenities.map((item, i) => (
            <div
              key={i}
              className='border px-4 py-2 rounded text-sm text-gray-600 flex items-center gap-2'
            >
              <span>🏨</span> {item}
            </div>
          ))}
        ...
        <div className='space-y-4'>
          {roomsList.map(
            (room) =>
              room.id != id && (
                ...
                >
                  <p className='text-2xl text-mainColor-400 font-semibold'>
                    {room.name}{' '}
                    <span className='text-sm text-mainColor1-400'>
                      (Max Guests: {room.max_guests})
                    </span>{' '}
                  </p>
                  <div className='text-right'>
                    <p className='text-lg text-mainColor1-800 font-semibold'>
                      ${room.price_per_night}
                      <span className='text-sm'>/night</span>
                    </p>
                    <button
                      onClick={handleViewRoom(room.id)}
                      className='bg-mainColor-400 text-white px-4 rounded hover:bg-mainColor-800 hover:scale-105 hover:drop-shadow-lg hover:duration-300 hover:ease-in-out transition-all py-2'
                    >
                      View
                    </button>
                  </div>
          ...
          {roomNum < maxRoomNum && (
            <div
              ...
              className='flex justify-center cursor-pointer bg-mainColor-400 text-white px-4 rounded-sm w-full hover:bg-mainColor-800  hover:drop-shadow-lg hover:duration-300 hover:ease-in-out transition-all py-2 mt-4'
            >
              Load More
            </div>
          )}
      ...
      <div className='!my-10'>
        <h2 className='text-xl font-semibold mb-4 text-mainColor-700'>
          Amenities
        </h2>
        <div className='grid grid-cols-2 md:grid-cols-3 gap-y-4 text-gray-700'>
          <div className='flex items-center gap-2'>
            <span>🏊</span> Outdoor pool
          </div>
          <div className='flex items-center gap-2'>
            <span>🏋️</span> Fitness center
          </div>
          <div className='flex items-center gap-2'>
            <span>🏠</span> Indoor pool
          </div>
          <div className='flex items-center gap-2'>
            <span>🍷</span> Bar/Lounge
          </div>
          <div className='flex items-center gap-2'>
            <span>💆</span> Spa and wellness center
          </div>
          <div className='flex items-center gap-2'>
          ...
          <div className='flex items-center gap-2'>
            <span>☕</span> Tea/coffee machine
          </div>
          <div className='flex items-center gap-2'>
            <span>🛎️</span> Room service
          </div>
        </div>
      </div>

      <div className='!mt-20'>
        <h2 className='text-xl font-semibold text-mainColor-700 mb-4'>
          Reviews
        </h2>
        ...
        <div className='flex items-center justify-between'>
          ...
          <Dialog>
            ...
            <DialogContent className='sm:max-w-md'>
              <DialogHeader>
                <DialogTitle className='text-3xl font-bold text-mainColor-600'>
                  Review
                </DialogTitle>
                <DialogDescription>
              ...
              <div className='flex flex-col mb-1'>
                <div className='text-lg font-semibold mb-1 text-mainColor1-700'>
                  Rating
                </div>
              ...
              <DialogFooter className='sm:justify-start'>
                <button
                  ...
                  className='bg-mainColor-600 text-white px-4 rounded hover:bg-mainColor-800 hover:scale-105 hover:drop-shadow-lg hover:duration-300 hover:ease-in-out transition-all py-2'
                >
                  Send
                </button>
                <DialogClose asChild>
      ...
      <div className='flex items-center justify-center mb-20'>
        <div className='w-[90%] md:w-[60%] px-10 py-16 bg-mainColor-200 flex flex-col md:flex-row items-center justify-center rounded-xl gap-6 shadow-md'>
          <div className='mb-6 md:mb-0'>
            <h2 className='text-3xl font-semibold mb-2 text-mainColor-700'>
          ...
          <form className='flex gap-2 w-full md:w-auto'>
            <input
              ...
              className='border !border-mainColor-600 hover:!border-2 hover:!border-mainColor-700 outline-mainColor-600 rounded-md p-2 placeholder:!text-mainColor-200 text-mainColor-800 w-full md:w-72'
            />
            <div className='bg-mainColor-600 text-white px-4 rounded w-full md:w-auto cursor-pointer hover:bg-mainColor-800 hover:scale-105 hover:drop-shadow-lg hover:duration-300 hover:ease-in-out transition-all py-2'>
              Subscribe
            </div>
          </form>
        </div>
